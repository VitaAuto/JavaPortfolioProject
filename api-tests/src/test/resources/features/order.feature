...
Feature: Order management
  ...
  Scenario: No orders with negative amounts exist
    Given user logs in with valid username and valid password
    When user creates new order with username "Andrew", description "Initial order comment", and amount "29.01"
  ...
  Scenario: Get all orders when at least 1 has been created
    ...
    When user tries to get all orders
    Then response should have status code 200
    And response should contain at least one order
    And no order should have negative amount
  ...
  Scenario Outline: Create a new order with valid data
    ...
    Then response should have status code 201
    And response should contain order with username "<username>", status CREATED, description "<description>", and amount <amount>
  ...
  Scenario Outline: Get order by id
    ...
    And response should contain order with id "<id>"
  ...
  Scenario Outline: Update order with valid data
    ...
    When user creates new order with username "<username>", description "<description>", and amount "<amount>"
    Then response should have status code 201
    When user updates order with id "<id>" and username "<username>", description "<description>", and amount <amount>
    Then response should have status code 200
    And response should contain order with username "<username>", status UPDATED, description "<description>", and amount <amount>
  ...
  Scenario Outline: Partial update order with valid data
    ...
    Then response should have status code 201
    When user partially updates fields of order with id "saved"
      | description | Patched order           |
      | amount      | 375.50                  |
    Then response should have status code 200
    And response should contain order with username "<initial_username>", status PARTIALLY_UPDATED, description "<updated_description>", and amount <updated_amount>
  ...
  Scenario: Delete order (soft)
    ...
    Then response should have status code 201
    When user deletes order with id "saved"
    Then response should have status code 200
    And response should contain order with username "User_To_Soft_Delete", status DELETED, description "To soft delete", and amount 50.00

  Scenario: Delete order (hard)
    Given user logs in with valid username and valid password
    When user creates new order with username "User_To_Hard_Delete", description "To hard delete", and amount "3001.00"
    Then response should have status code 201
    When user hard deletes order with id "saved"
    Then response should have status code 204

  Scenario: Patch order with invalid field
    Given user logs in with valid username and valid password
    When user creates new order with username "User for PATCH", description "InitialInvalid", and amount "1.00"
    Then response should have status code 201
    When user partially updates fields of order with id "saved"
      | field      | value           |
      | unknown    | test            |
